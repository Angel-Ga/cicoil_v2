...
if __name__ == "__main__":
  ...
  # o.set_config('drift:vertical_mixing',True)#modf
  # depths=o.vertical_mixing(store_depths=True)#modf
  ...
  # load TAMOC files
  tamoc_file= ''.join((output_tamoc, PtoParam['input']['curr'], '-tamoc_', txtTime, '_', PtoParam['point']['name'],'_',str(release_depth.as_integer_ratio()[0]),'m','_',PtoParam['oil']['name']))
  ...
  for day in range(leak_days):
      ...
      o.seed_plume_elements(lon, lat, tamoc_plume, starttime+ timedelta(days=day), stoptime,
                               number=spillets, z_uncertainty=10,z=-10, wind_drift_factor=wind_factor,
                               oiltype=oil_name)
  reader_winds.buffer = 10
  ...
  # Maps
  o.plot(filename=postp_file +'_trajectories.png')
  o.plot(corners=[-98, -90, 18, 23] ,filename=postp_file + '_trajectorias_costa.png')
  ...
  o.animation(corners=[-98, -90, 18, 23], background=['x_sea_water_velocity', 'y_sea_water_velocity'],
              colorbar=True, fps=8, filename=postp_file + '_current_costa.gif')
  o.animation(color='viscosity', fsp=8, filename=postp_file + '_viscosity.gif') # Added by "abgarcia"
  o.animation(corners=[-98, -90, 18, 23], color='viscosity', fsp=8, filename=postp_file + '_viscosity_costa.gif')
  o.animate_vertical_distribution(filename=postp_file + '_vertical.gif')#modif
